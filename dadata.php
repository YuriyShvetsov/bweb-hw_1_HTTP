...
// Стандартизовать ФИО
function printTable(array $result):void {
    ?>
    <table>
        <tbody>
        <tr>
        ...
        <tr>
            <td>ФИО (кого?)</td>
            <td><?=$result[0]['result_genitive']?></td>
        </tr>
        <tr>
            <td>ФИО (кому?)</td>
            <td><?=$result[0]['result_dative']?></td>
        </tr>
        <tr>
            <td>ФИО (кем?)</td>
            <td><?=$result[0]['result_ablative']?></td>
        </tr>
        <tr>
            <td>Фамилия</td>
...
function getData($arData) {
    $secret = $arData['secret'];
    $token = $arData['token'];
    $dadata = new Dadata($token, $secret);
    $dadata->init();
    $result = $dadata->clean("name", $arData['user_name']." ".$arData['user_second_name']." ".$arData['user_last_name']);
    printTable($result);
    $dadata->close();
}
